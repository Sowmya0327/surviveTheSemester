...
export class CannonDuelGame {
  ...
  bindSocketEvents() {
    ...
    this.multiplayer.on("player-shot", ({ side, angle, power, wind, shotId }) => {
      this.wind = wind;
      if (side === this.localSide && this.projectiles.some((projectile) => projectile.id === shotId)) {
        return;
      }
    ...
    this.multiplayer.on("player-ready", ({ readyCount }) => {
      this.ui.setRoomStatus(`Players ready: ${readyCount}/2`, true);
    });

    this.multiplayer.on("player-joined", () => {
      this.ui.setRoomStatus(`Room ${this.roomId} is full. Press Ready when you are set.`, true);
    });
  ...
  onPointerDown() {
    ...
    this.audio.ensure();
    this.getCannon(this.localSide).setFrozen(true);
  ...
  update(dt) {
    ...
    if (this.mode !== "online" || this.isRoundAuthority()) {
      ...
      if (this.mode === "online") {
        for (const projectile of this.projectiles) {
          this.multiplayer.sendProjectileUpdate({
            id: projectile.id,
            x: projectile.x,
            y: projectile.y,
            vx: projectile.vx,
