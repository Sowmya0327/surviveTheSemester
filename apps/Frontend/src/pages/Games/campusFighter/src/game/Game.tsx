import { Collisions, Constants, Entities, Geometry, Maps, Maths, Models, Tiled, Types } from '../../../common/src';
...
export class Game {
    ...
    // Inputs
    public forcedRotation: number | null = null; // Used on mobile only
    ...
    playerAdd = (playerId: string, attributes: Models.PlayerJSON, isMe: boolean) => {
        ...
        // ADD THIS: Hide the server's ghost if it is us, so we don't see double!
        if (isMe) {
        ...
        // If the player is "you", create the local controllable character
        if (isMe) {
    ...
    playerUpdate = (playerId: string, attributes: Models.PlayerJSON, isMe: boolean) => {
        if (isMe && this.me) {
            ...
            // FIX: Update the Ghost health OUTSIDE the ack block so the HUD always sees it!
            ghost.lives = attributes.lives;
            ghost.kills = attributes.kills;

            // Only snap position if we have a new server acknowledgement
            if (attributes.ack !== this.me.ack) {
                ...
                // 2. Sync the Ghost's health so the HUD actually shows the damage!
                ghost.lives = attributes.lives;
                ghost.kills = attributes.kills;

                // 3. Drop all old inputs that the server already processed
                //@ts-ignore
                this.moveActions = this.moveActions.filter((action) => action.ts > attributes.ack);

                // 4. Re-apply any new WASD inputs that the server hasn't seen yet
                ...
                // 5. Finally, snap our visible character to the correct calculated position
                this.me.x = ghost.x;
                this.me.y = ghost.y;
    ...
    propUpdate = (propId: string, attributes: Models.PropJSON) => {
        ...
        // FIX: Hide the potion from the map if it was consumed!
        prop.container.visible = attributes.active;
    };
